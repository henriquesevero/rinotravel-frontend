 ...
 */
export function useDayMap(tripId: string, stops: Stop[], mode: DayMode, includeImage: boolean) {
  return useQuery({
    enabled: stops.length >= 2,
    queryKey: ['day-map', tripId, stopsSignature(stops), mode, includeImage],
    queryFn: ({ signal }) =>
      unwrap(
        api.POST('/api/v1/trips/{tripId}/maps/day', {
          params: { path: { tripId } },
          body: { stops: toRequestStops(stops), mode, language: currentLocale(), includeImage },
          signal,
        }),
      ),
    staleTime: 30 * 60_000,
