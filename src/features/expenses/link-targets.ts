 ...
 * things of the itinerary, tickets and hotels. It reads the lists the other screens already keep.
 */
export function useLinkTargets(tripId: string) {
  ...
  return {
    ...
    isPending: [places, restaurants, items, tickets, hotels].some((query) => query.isPending),
  };
}
